...
* From your deployment space overview, in the table, find the model name for the model you previously built and now want to create a deployment against. Use your mouse to hover over the right side of that table row and click the `Deploy` rocket icon (the icons are not visible by default until you hover over them).
  ...
  [![Actions Deploy model](../images/deployment/deploy-spark-model.png)](../images/deployment/deploy-spark-model-button.png)
...
* Back on the Cloud Pak for Data model deployment page, gather the `URL` to invoke the deployed model from the *API reference* by copying the `Endpoint` (an example endpoint would be: *`"https://us-south.ml.cloud.ibm.com/ml/v4/deployments/<DEPLOYMENT_ID>/predictions?version=2020-09-01"`*).

  [![Model Deployment Endpoint](../images/deployment/deploy-model-endpoint.png)](../images/deployment/deploy-model-endpoint.png)

* Now save that endpoint to a variable named `URL` in your terminal by exporting it. (*Note: the URL should end with a version query parameter*).
